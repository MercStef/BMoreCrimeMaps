...
function normalizeFeature(f, source) {
  ...
  const districtRaw = source === 'nibrs' ? a.New_District : a.DISTRICT_1

  const district = typeof districtRaw === 'string' ? districtRaw.trim().toUpperCase() : null

  const rawDate = source === 'nibrs' ? a.CrimeDateTime : a.Date_

  let date = typeof rawDate === 'string' ? Date.parse(rawDate) : typeof rawDate === 'number' ? rawDate : null
  if (isNaN(date)) date = null

  const description = source === 'homicide' ? a.NBRDESC || a.WEAPON || 'HOMICIDE' : a.Description || 'UNKNOWN'
...
async function runBackup() {
  ...
  await fs.writeFile('./public/data/crime-backup.json', JSON.stringify(output, null, 2))
