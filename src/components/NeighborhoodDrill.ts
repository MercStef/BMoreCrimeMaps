export class NeighborhoodDrill {
  ...
  constructor(rootId: string) {
    ...
    if (!rootElement) {
      throw new Error(`NeighborhoodDrill: Required DOM element not found: #${rootId}`);
    }
  ...
  private q(el: string): HTMLElement {
    const elem = this.root.querySelector(`[data-el="${el}"]`) as HTMLElement | null;
    if (!elem) {
      throw new Error(`NeighborhoodDrill: Required child element not found: [data-el="${el}"]`);
    }
    return elem;
  }

  public update(mode: "heatmap" | "choropleth", selectedFeature: any | null): void {
    this.mode = mode;
